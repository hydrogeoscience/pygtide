...
!#######################################################################
      module PARAMS
          ...
          ! commdat directory
          CHARACTER(10), PARAMETER :: COMDIR='commdat'
          ! syste dependent path seperator
          CHARACTER(1), PARAMETER :: PATHSEP='\'
          ! names of the i/o files
          CHARACTER(16), PARAMETER :: CFINI='pytide.waves.ini',CFPRN='pytide.out.prn',CFOUT='pytide.out.prd',&
...
!#######################################################################
      ...
      module INOUT
          ...
          CHARACTER(8) :: ETPUNIT
          !CHARACTER(6), PARAMETER :: CREST='PyTide'
          CHARACTER(6), PARAMETER :: CPROJ='pytide'
...
! ##################################################################
! implement function INPUT and OUTPUT for F2PY
      PROGRAM ETERNA_PREDICT
          use PARAMS
          REAL(8), DIMENSION(18) :: ARGS
          ARGS = 0D0
!     'SENSORNAME' CREST (parameter set in module out)
!     'STATLATITU' DLAT
          ARGS(1) = -33.33D0
!     'STATLONITU') DLON
          ARGS(2) = 151.33D0
...
!     'SAMPLERATE' IDTSEC
          ARGS(8) = 900
! STATGRAVIT default 0
          ARGS(9) = 0
! STATAZIMUT default 0
          ARGS(10) = 0
! TIDALPOTEN deafult 8
          ARGS(11) = 8
! TIDALCOMPO default 0
          ARGS(12) = 0
! AMTRUNCATE default 1.D-8
          ARGS(13) = 1.D-10
! POLTIDECOR default 1.16D0
          ARGS(14) = 0D0
! LODTIDECOR default 1.16D0
          ARGS(15) = 0D0
! FILEPRD default 0
          ARGS(16) = 1D0
! FILEPRN default 0
...
! GCR  this subroutne sets the variables in module 'INOUT' for access in Python
      subroutine INIT
        ...
        OPEN(UNIT=IUN30,FILE=TRIM(COMDIR)//PATHSEP//TRIM(ETPOLUTDAT),FORM='FORMATTED',&
...
!-GCR this is the maian interface for f2py
...
!-GCR read wave group parameters from ini file
      OPEN(IUN15, FILE=TRIM(COMDIR)//PATHSEP//TRIM(CFINI),ACTION='READ',FORM='FORMATTED')
!-GCR continue ...
...
!-GCR define the size of the output matrix
      ROWS = CEILING(REAL(ISPANH)/(REAL(IDTSEC)/3600))
...
!#######################################################################
      OPEN(UNIT=IUN30,FILE=TRIM(COMDIR)//PATHSEP//TRIM(ETPOLUTDAT),FORM='FORMATTED',STATUS='OLD')
      DCLAT=COS(DLAT*DRAD)
      DSLAT=SIN(DLAT*DRAD)
      DCLON=COS(DLON*DRAD)
      DSLON=SIN(DLON*DRAD)
...
!-GCR fill new array for handback to Python
      ...
      IF (ROWI.EQ.1) THEN
        ...
        HEADER(3,:)=CHANNEL(1) //'[' // ETPUNIT // ']'
        HEADER(4,:)=CHANNEL(2) //'[' // ETPUNIT // ']'
        HEADER(5,:)=CHANNEL(3) //'[' // ETPUNIT // ']'
        HEADER(6,:)=CHANNEL(4) //'[' // ETPUNIT // ']'
        !WRITE (*,'(A20)') (HEADER(JC,:),JC=1,6)
      ENDIF
      ! advance row counter
      ROWI=ROWI+1
!-GCR end modification
...
!#######################################################################
      ...
      SAVE
      OPEN(UNIT=IUN27,FILE=TRIM(COMDIR)//PATHSEP//TRIM(ETDDTDAT),STATUS='OLD')
...
!#######################################################################
      OPEN(UNIT=IUN31,FILE=TRIM(COMDIR)//PATHSEP//TRIM(ETPOLUTBIN), &
...
!      REWIND IUN31
      ...
      ISTART=0
      GOTO 1000
   11 OPEN(UNIT=IUN31,FILE=TRIM(COMDIR)//PATHSEP//TRIM(ETPOLUTBIN), &
...
!& increase cmodel
      IF(IPRINT.GT.0) WRITE(IUN16,17001) CMODEL(IMODEL)
      OPEN(UNIT=IUN14,FILE=TRIM(COMDIR)//PATHSEP//TRIM(CFFILE(IMODEL)),FORM='FORMATTED',STATUS='OLD')
      REWIND(IUN14)
!#######################################################################
!     Test, whether there exist already the unformatted tidal
!     potential catalogue file:
!#######################################################################
      OPEN(UNIT=IUN24,FILE=TRIM(COMDIR)//PATHSEP//TRIM(CUFILE(IMODEL)),FORM='UNFORMATTED',STATUS='OLD',ERR=11)
      LEX24=.TRUE.
      REWIND IUN24
      GOTO 12
   11 OPEN(UNIT=IUN24,FILE=TRIM(COMDIR)//PATHSEP//TRIM(CUFILE(IMODEL)),FORM='UNFORMATTED',STATUS='NEW')
      LEX24=.FALSE.
      REWIND IUN14
   12 CONTINUE
!#######################################################################
